...
const inputPath = path.join(repoRoot, 'docs', 'ARCHITECTURE.en.md');
const tmpDir = path.join(repoRoot, 'tmp', 'pdfs');
const outputDir = path.join(repoRoot, 'output', 'pdf');
const htmlPath = path.join(tmpDir, 'architecture-en.html');
const pdfPath = path.join(outputDir, 'architecture-en.pdf');
const screenshotPath = path.join(tmpDir, 'architecture-en-preview.png');
...
<head>
  ...
  <title>Bentix Architecture (English)</title>
  <style>${css}</style>
</head>
<body>
  <main>
    <div class="meta">Document generated from <code>docs/ARCHITECTURE.en.md</code>.</div>
    ${blocks.join('\n')}
  </main>
</body>
</html>`;
