...
import { getWorkPlanById } from './work-plans.js'
...
export function resolvePreviewScopedSession(session, searchParams) {
...
export function getChefReferenceAssignments(session, filters = {}) {
...
export function extendDefaultsForChef(defaults, session, filters = {}) {
...
export function isChefPersonAllowedForWork(session, { workPlanId, date, workId, personId }) {
  if (!session || !isChefRole(session.role)) {
    return true
  }
...
export function resolveDailyPlanDate({ workPlanId, date }) {
  if (date) {
    return date
  }
