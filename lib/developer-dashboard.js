...
const STATUS_LABELS = {
  ...
  [WorkStatus.COMPLETED]: 'Concluidas',
}
...
function buildIssues({ people, identities, assignments, notes, duplicates }) {
  ...
  if (brokenIdentities.length > 0) {
    issues.push({
      severity: 'high',
      title: 'Acessos com ligacoes invalidas',
  ...
  if (missingManagerCredentials.length > 0) {
    issues.push({
      severity: 'medium',
      title: 'Chefes ou responsaveis sem login',
      description: `${missingManagerCredentials.length} pessoas com acesso a app ainda nao tem credenciais operacionais. ${buildPreview(missingManagerCredentials, person => person.name)}`,
    })
  }
  ...
  if (chefIdentitiesWithoutWorks.length > 0) {
    issues.push({
      ...
      description: `${chefIdentitiesWithoutWorks.length} contas de chef nao tem obras atribuidas. ${buildPreview(chefIdentitiesWithoutWorks, identity => identity.username)}`,
    })
  }
  ...
  if (assignmentsWithoutCost.length > 0) {
    issues.push({
      severity: 'medium',
      title: 'Afetacoes com custo horario a zero',
      description: `${assignmentsWithoutCost.length} afetacoes registam horas sem custo horario definido. ${buildPreview(assignmentsWithoutCost, assignment => assignment.person?.name)}`,
    })
  }
  ...
  if (notesWithoutWork.length > 0) {
    issues.push({
      ...
      description: `${notesWithoutWork.length} notas diarias perderam a referencia da obra.`,
    })
  }
...
export function getDeveloperDashboardData() {
  ...
  return {
    metrics: [
      ...
      { label: 'Planos diarios', value: workPlans.length, helper: 'Calendario de trabalho' },
      { label: 'Afetacoes', value: assignments.length, helper: `${assignments.filter(assignment => assignment.submitted).length} submetidas` },
      { label: 'Notas diarias', value: notes.length, helper: `${recentEvents.length} eventos recentes` },
      ...
      { label: 'Contas tecnicas', value: admins.length + developers.length, helper: `${developers.length} developer / ${admins.length} admin` },
    ],
    ...
    peopleSummary: [
      ...
      { label: 'Programadores', value: developers.length, helper: 'Gestao tecnica' },
      { label: getRoleLabel(ROLE_RESPONSAVEL), value: managerCount, helper: 'Perfis de aprovacao' },
    ...
    accessSummary: [
      ...
      { label: 'Alertas tecnicos', value: issues.length, helper: issues.length > 0 ? 'Requer revisao' : 'Estado limpo' },
      { label: 'Usernames duplicados', value: duplicates.length, helper: 'Evitar colisao de login' },
      { label: 'Eventos recentes', value: recentEvents.length, helper: 'Ultima atividade visivel' },
      { label: 'Historico de logins', value: loginSummary.totalLogins, helper: 'Registos guardados no sistema' },
    ],
    issues,
    loginSummary,
    recentEvents,
