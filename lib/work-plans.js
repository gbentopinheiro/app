...
function upsertWorkPlanMirror(workPlanData) {
  workPlansService.refresh()

  const normalizedWorkPlan = new WorkPlan(workPlanData)
  const existingIndex = workPlansService.workPlans.findIndex(workPlan => workPlan.id === normalizedWorkPlan.id)

  if (existingIndex >= 0) {
    workPlansService.workPlans[existingIndex] = normalizedWorkPlan
  } else {
    workPlansService.workPlans.push(normalizedWorkPlan)
  }

  workPlansService.save()
...
function removeWorkPlanMirror(id) {
  ...
  const existingIndex = workPlansService.workPlans.findIndex(workPlan => workPlan.id === normalizedId)

  if (existingIndex === -1) {
    return false
  }

  workPlansService.workPlans.splice(existingIndex, 1)
  workPlansService.save()
  return true
}
...
export async function createWorkPlanData(data) {
  ...
  try {
    const workPlan = await createWorkPlanDb(data)
    return upsertWorkPlanMirror(workPlan)
  } catch (error) {
...
export async function updateWorkPlanData(id, data) {
  ...
  try {
    const workPlan = await updateWorkPlanDb(id, data)
...
export async function deleteWorkPlanData(id) {
  ...
  const deleted = await deleteWorkPlanDb(id)

  if (!deleted) {
    return false
  }

  removeWorkPlanMirror(id)
  return true
}
