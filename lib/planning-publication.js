...
import {
  normalizeAssignmentPurpose,
  shouldAssignmentBehaveAsPlanningAssignment,
...
import { getWorkExtraAccessSelectionsByPersonData } from './work-extra-access-grants.js'
...
async function buildPlanningWorkspaceView(date, companyId) {
  ...
  const workExtraAccessSelectionsByPersonId = await getWorkExtraAccessSelectionsByPersonData({
    personIds: (defaults.people || []).map(person => person.id),
    workIds: (defaults.works || []).map(work => work.id),
  })
  ...
  if (!workspace) {
    return {
      workspace: null,
      items: [],
      workExtraAccessSelectionsByPersonId,
      defaults: {
...
export async function initializePlanningWorkspaceDraftData({
  ...
  if (existingWorkspace && onlyIfMissing) {
    return {
      ...
      items: await getPlanningWorkspaceAssignmentsData({ workspaceId: existingWorkspace.id }),
    }
  }
...
export async function createPlanningDraftAssignmentData(workspaceId, payload = {}) {
  ...
  const assignment = await createPlanningWorkspaceAssignmentData({
    ...
    assignmentPurpose: normalizeAssignmentPurpose(payload.assignmentPurpose),
  })
...
export async function updatePlanningDraftAssignmentData(assignmentId, payload = {}) {
  ...
  const updatedAssignment = await updatePlanningWorkspaceAssignmentData(assignmentId, {
    ...
    assignmentPurpose:
      payload.assignmentPurpose !== undefined
        ? normalizeAssignmentPurpose(payload.assignmentPurpose)
  ...
  for (const draftAssignment of draftAssignments) {
    ...
    if (!officialAssignment) {
      await createWorkAssignmentData({
        ...
        assignmentPurpose: normalizeAssignmentPurpose(draftAssignment.assignmentPurpose),
