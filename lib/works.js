...
function upsertWorkMirror(workData) {
  worksService.refresh()

  const normalizedWork = new Work(workData)
  const existingIndex = worksService.works.findIndex(work => work.id === normalizedWork.id)

  if (existingIndex >= 0) {
    worksService.works[existingIndex] = normalizedWork
  } else {
    worksService.works.push(normalizedWork)
  }
...
function removeWorkMirror(id) {
  ...
  const existingIndex = worksService.works.findIndex(work => work.id === normalizedId)

  if (existingIndex === -1) {
    return false
  }

  worksService.works.splice(existingIndex, 1)
  worksService.save()
  return true
}
