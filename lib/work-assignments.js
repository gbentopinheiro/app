...
function upsertWorkAssignmentMirror(assignmentData) {
  workAssignmentsService.refresh()

  const normalizedAssignment = new WorkAssignment(assignmentData)
  const existingIndex = workAssignmentsService.assignments.findIndex(
    assignment => assignment.id === normalizedAssignment.id,
  )

  if (existingIndex >= 0) {
    workAssignmentsService.assignments[existingIndex] = normalizedAssignment
  } else {
    workAssignmentsService.assignments.push(normalizedAssignment)
  }

  workAssignmentsService.save()
...
function removeWorkAssignmentMirror(id) {
  ...
  workAssignmentsService.assignments.splice(existingIndex, 1)
  workAssignmentsService.save()
  return true
}
...
async function validateNoDuplicatePersonWorkData(workPlanId, personId, workId, excludeAssignmentId = null) {
  const matchingAssignments = await getAllWorkAssignmentsDb({ workPlanId, personId, workId })
  validateNoDuplicatePersonWork(matchingAssignments, workPlanId, personId, workId, excludeAssignmentId)
}
...
async function syncChefAccessForWorkPlanData(workPlanId) {
  workAssignmentsService.refresh()
  syncChefAccessForWorkPlan(workAssignmentsService.assignments, workPlanId)
}
...
export function repriceWorkAssignmentsForWork(workId, startDate) {
  return workAssignmentsService.repriceForWork(workId, startDate)
}
...
export async function createWorkAssignmentData(data) {
  ...
  await validateNoDuplicatePersonWorkData(workPlanId, data.personId, data.workId)
  ...
  const scopedAssignments = await syncWorkAssignmentMirrorsForScope(assignment.workPlanId, assignment.workId)

  if (!assignment.hasWorkAccess) {
    upsertWorkAssignmentMirror(assignment)
  }
...
export async function updateWorkAssignmentData(id, data, options = {}) {
  ...
  validateCompanyConsistencyData(workPlan, work, person)
  await validateNoDuplicatePersonWorkData(workPlanId, personId, workId, currentAssignment.id)
  ...
  if (
    Number(currentAssignment.workPlanId) !== Number(updatedAssignment.workPlanId) ||
    Number(currentAssignment.workId) !== Number(updatedAssignment.workId)
  ) {
    await syncWorkAssignmentMirrorsForScope(currentAssignment.workPlanId, currentAssignment.workId)
  }

  const scopedAssignments = await syncWorkAssignmentMirrorsForScope(
    updatedAssignment.workPlanId,
    updatedAssignment.workId,
  )

  if (!updatedAssignment.hasWorkAccess) {
    upsertWorkAssignmentMirror(updatedAssignment)
  }
