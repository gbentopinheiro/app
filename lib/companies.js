...
export async function createCompanyData(data) {
  ...
  try {
    const company = await createCompanyDb(data)
    return upsertCompanyMirror(company)
  } catch (error) {
...
export async function updateCompanyData(id, data) {
  ...
  try {
    const company = await updateCompanyDb(id, data)
...
export async function deleteCompanyData(id) {
  ...
  const deleted = await deleteCompanyDb(id)

  if (!deleted) {
    return false
  }

  return removeCompanyMirror(id)
}
