...
function upsertClientMirror(clientData) {
  clientsService.refresh()

  const normalizedClient = new Client(clientData)
  const existingIndex = clientsService.clients.findIndex(client => client.id === normalizedClient.id)

  if (existingIndex >= 0) {
    clientsService.clients[existingIndex] = normalizedClient
  } else {
    clientsService.clients.push(normalizedClient)
  }
...
function removeClientMirror(id) {
  ...
  const existingIndex = clientsService.clients.findIndex(client => client.id === normalizedId)

  if (existingIndex === -1) {
    return false
  }

  clientsService.clients.splice(existingIndex, 1)
  clientsService.save()
  return true
}
...
export async function createClientData(data) {
  ...
  const client = await createClientDb(data)
  upsertClientMirror(client)
  return client
}
