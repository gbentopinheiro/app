...
import { buildLoginRedirectPath } from './safe-redirect.js'
...
export function isPublicAppPath(pathname) {
  return pathname === '/login' || pathname === '/mobile/login' || pathname.startsWith('/api/auth/')
}

export function isMobileAppPath(pathname) {
  return pathname === '/mobile' || hasPathPrefix(pathname, '/mobile')
}

export function getLoginPathForPath(pathname) {
  return isMobileAppPath(pathname) ? '/mobile/login' : '/login'
}

export function getUnauthorizedRedirectPath(pathname, search = '') {
  const loginPath = getLoginPathForPath(pathname)

  if (isPublicAppPath(pathname)) {
    return loginPath
  }

  if (loginPath !== '/mobile/login') {
    return loginPath
  }

  return buildLoginRedirectPath(`${pathname}${search}`, loginPath)
}
...
function getSessionCookieDomain(rawDomain = process.env.SESSION_COOKIE_DOMAIN) {
  const domain = String(rawDomain || '').trim()
  return domain || undefined
}

export function getSessionCookieBaseOptions({
  rawDomain = process.env.SESSION_COOKIE_DOMAIN,
  nodeEnv = process.env.NODE_ENV,
} = {}) {
  const domain = getSessionCookieDomain(rawDomain)
  const options = {
    httpOnly: true,
    sameSite: domain ? 'none' : 'lax',
    secure: domain ? true : nodeEnv === 'production',
    path: '/',
  }
...
export function getSessionCookieOptions(overrides = {}) {
  return {
    ...getSessionCookieBaseOptions(overrides),
    maxAge: SESSION_MAX_AGE_SECONDS,
  }
}

export function getExpiredSessionCookieOptions(overrides = {}) {
  return {
    ...getSessionCookieBaseOptions(overrides),
    expires: new Date(0),
  }
...
export function canDeveloperAccessPath(pathname) {
  return (
    ...
    pathname === '/api/activity-history/export' ||
    pathname === '/account-settings' ||
    pathname === '/api/account/password' ||
...
export function canAccessPathByPermission(session, pathname) {
  ...
  if (pathname === '/activity-history' || pathname === '/api/activity-history/export') {
