...
import { ROLE_CHEF_PRIMEIRA, getRoleLabel, isSupportedRole, normalizeRole, roleUsesWorkScope } from './roles.js'
import { getAllWorks, getAllWorksData, getWorkById } from './works.js'
...
function buildLegacyIdentityIndex(identities = accessIdentitiesService.getAll()) {
  return {
    byId: new Map(
      identities
...
function getLegacyIdentityForDbUser(user, index = null) {
  ...
  return (
    (normalizedLegacySourceId ? accessIdentitiesService.getById(normalizedLegacySourceId) : null) ||
    (normalizedPersonId ? accessIdentitiesService.getByPersonId(normalizedPersonId) : null) ||
    (normalizedUsername ? accessIdentitiesService.getByUsername(normalizedUsername) : null) ||
    null
  )
}
...
async function resolveDbIdentityWorks(user, legacyIdentity = null) {
  ...
  const assignments = (await getAllWorkAssignmentsDb({ personId: user.personId }))
    .filter(assignment => assignment.hasWorkAccess === true && assignment.work)

  if (assignments.length === 0) {
    return []
  }

  const assignmentsWithDate = assignments.filter(assignment => assignment.date)
  const latestDate = assignmentsWithDate
    .map(assignment => String(assignment.date))
    .sort((left, right) => right.localeCompare(left))[0]
  const scopedAssignments = latestDate
    ? assignments.filter(assignment => String(assignment.date || '') === latestDate)
    : assignments
...
function rollbackLegacyIdentity(identityId, snapshot, options = {}) {
  ...
  if (!snapshot) {
    accessIdentitiesService.delete(identityId)
    return
  }

  accessIdentitiesService.update(identityId, snapshot, {
    ...options,
    enforcePolicy: false,
  })
}
...
export async function createAccessIdentityData(data, options = {}) {
  ...
  try {
    ...
    const payload = validateIdentityDbPayload(
      {
        ...data,
        passwordHash,
      },
