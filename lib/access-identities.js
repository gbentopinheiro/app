...
import { ROLE_CHEF, getRoleLabel, isSupportedRole, normalizeRole } from './roles.js'
  ...
  load() {
    ...
    if (existsSync(legacyChefsFilePath)) {
      try {
        ...
        return normalizeIdentities(rawData.map(identity => ({ ...identity, role: ROLE_CHEF })))
      } catch (error) {
...
export function createAccessIdentity(data) {
  return accessIdentitiesService.create(data)
}

export function updateAccessIdentity(id, data) {
  return accessIdentitiesService.update(id, data)
}
