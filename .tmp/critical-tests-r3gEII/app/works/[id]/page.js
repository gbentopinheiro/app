...
import { BentixButton, BentixContent, BentixPage, BentixResponsiveGrid } from '../../components/ViewportLayout.js'
...
function buildWorkExportExcelWorkbook(months, work, workId) {
  ...
  months.forEach(month => {
    month.days.forEach(day => {
      assignmentDates.push(day.date)

      day.people.forEach(assignment => {
        assignments.push({
          ...assignment,
          date: assignment?.date || day.date,
        })
      })
    })
  })

  const sortedDates = assignmentDates
    .map(date => String(date || '').slice(0, 10))
    .filter(Boolean)
    .sort((left, right) => left.localeCompare(right))
  const startDate = sortedDates[0] || selectedExportMonthToDate(months, 'start')
  const endDate = sortedDates[sortedDates.length - 1] || selectedExportMonthToDate(months, 'end')
...
function selectedExportMonthToDate(months, edge) {
  const monthKey = String(months?.[0]?.monthKey || '').slice(0, 7)
...
export default function WorkDetailPage() {
  ...
  async function exportExcel() {
    ...
    try {
      ...
      const workbookBytes = buildWorkbookBytes(workbook)
  ...
  function exportPdf() {
    ...
    try {
      ...
      const sortedDates = assignmentDates
        ...
        .filter(Boolean)
        .sort((left, right) => left.localeCompare(right))
      const startDate = sortedDates[0] || selectedExportMonthToDate(monthsToExport, 'start')
      const endDate = sortedDates[sortedDates.length - 1] || selectedExportMonthToDate(monthsToExport, 'end')
      const html = buildSingleWorkSummaryPrintDocument({
        work,
        workId,
        assignments,
        startDate,
          ...
          {!loading && !error && work && (
            ...
            <section style={panelStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '12px' }}>
                <h2 style={{ margin: 0 }}>Afetações da obra este mês</h2>
