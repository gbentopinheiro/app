...
import ClientReportsModal from '../components/ClientReportsModal.js'
import CompanyReportsModal from '../components/CompanyReportsModal.js'
...
import {
  exportClientWorkSummary,
  triggerDownload,
} from '../../frontend/controllers/work-summary-export-controller.js'
import {
...
function formatPdfMonthTitleCase(label) {
...
function clampPdfLayoutMetric(value, min, max) {
  return Math.min(Math.max(Number(value) || 0, min), max)
}

function splitPdfTableColumns(columns, maxWidthMm, stickyFirstColumnCount = 1) {
  if (!Array.isArray(columns) || columns.length === 0) {
    return []
  }

  const safeStickyFirstColumnCount = Math.max(
    0,
    Math.min(Number(stickyFirstColumnCount) || 0, Math.max(columns.length - 1, 0)),
  )

  if (safeStickyFirstColumnCount === 0) {
    return [columns]
  }

  const stickyColumns = columns.slice(0, safeStickyFirstColumnCount)
  const remainingColumns = columns.slice(safeStickyFirstColumnCount)
  const stickyWidthMm = stickyColumns.reduce(
    (totalWidth, column) => totalWidth + Number(column.minWidthMm || 24),
    0,
  )

  if (remainingColumns.length === 0 || stickyWidthMm >= maxWidthMm) {
    return [columns]
  }

  const columnGroups = []
  let currentGroup = [...stickyColumns]
  let currentWidthMm = stickyWidthMm

  for (const column of remainingColumns) {
    const columnWidthMm = Number(column.minWidthMm || 24)
    const wouldOverflow = currentGroup.length > stickyColumns.length && currentWidthMm + columnWidthMm > maxWidthMm

    if (wouldOverflow) {
      columnGroups.push(currentGroup)
      currentGroup = [...stickyColumns, column]
      currentWidthMm = stickyWidthMm + columnWidthMm
      continue
    }

    currentGroup.push(column)
    currentWidthMm += columnWidthMm
  }
  ...
  return columnGroups.length > 0 ? columnGroups : [columns]
}

function buildPdfReportTableLayout(columns, options = {}) {
  ...
  const horizontalMarginMm = clampPdfLayoutMetric(
    options.horizontalMarginMm ?? (
      columnCount >= 10 ? 7
        : columnCount >= 8 ? 8
          : columnCount >= 6 ? 9
            : 10
  ...
  const topMarginMm = clampPdfLayoutMetric(options.topMarginMm ?? 11, 10, 16)
  const bottomMarginMm = clampPdfLayoutMetric(options.bottomMarginMm ?? 16, 14, 22)
  const cellPaddingY = clampPdfLayoutMetric(
    options.cellPaddingY ?? (columnCount >= 8 ? 6 : columnCount >= 6 ? 7 : 8),
    5,
    10,
  )
  const cellPaddingX = clampPdfLayoutMetric(
    options.cellPaddingX ?? (columnCount >= 8 ? 7 : columnCount >= 6 ? 8 : 10),
    6,
    12,
  )
  const tableFontSizePx = clampPdfLayoutMetric(
    options.tableFontSizePx ?? (columnCount >= 8 ? 11 : columnCount >= 6 ? 11.5 : 12),
    11,
    13,
  )
  const headerFontSizePx = clampPdfLayoutMetric(
    options.headerFontSizePx ?? Math.max(9.5, tableFontSizePx - 2),
    9.5,
    11,
  )
  const printableWidthMm = Math.max(pageWidthMm - (horizontalMarginMm * 2), 110)
  const horizontalPaddingAllowanceMm = safeColumns.length * cellPaddingX * 0.45
  const estimatedContentWidthMm = safeColumns.reduce(
    (totalWidth, column) => totalWidth + Number(column.minWidthMm || 24),
    0,
  )
  const maxContentWidthMm = Math.max(printableWidthMm - horizontalPaddingAllowanceMm, 88)
  const stickyFirstColumnCount = Math.max(
    0,
    Math.min(Number(options.stickyFirstColumnCount ?? 1) || 0, Math.max(safeColumns.length - 1, 0)),
  )
  const rawColumnGroups = splitPdfTableColumns(safeColumns, maxContentWidthMm, stickyFirstColumnCount)
  const columnGroups = rawColumnGroups.map(group => {
    const totalWeight = group.reduce((weightTotal, column) => weightTotal + Number(column.weight || 1), 0) || group.length

    return group.map(column => ({
      ...column,
      widthPercent: ((Number(column.weight || 1) / totalWeight) * 100).toFixed(2),
      alignClass:
        column.align === 'right'
          ? 'cell-align-right'
          : column.align === 'center'
  ...
  return {
    ...
    tableFontSizePx,
    headerFontSizePx,
    requiresColumnSplit: estimatedContentWidthMm > maxContentWidthMm,
    columnGroups,
  }
}

function renderPdfReportTableSections({ columnGroups, rows, footerRow }) {
  return columnGroups.map((columnGroup, groupIndex) => {
    const colgroupMarkup = columnGroup
      .map(column => `<col style="width: ${column.widthPercent}%;" />`)
      .join('')

    const headerCells = columnGroup
      .map(column => `<th class="${column.alignClass}">${escapeHtml(column.label)}</th>`)
      .join('')

    const bodyRows = rows
      .map(row => `
        <tr>
          ${columnGroup.map(column => `<td class="${column.alignClass}">${column.render(row)}</td>`).join('')}
        </tr>
      `)
      .join('')

    const footerCells = footerRow
      ? columnGroup.map(column => `<td class="${column.alignClass}">${column.render(footerRow)}</td>`).join('')
      : ''

    return `
      <section class="report-table-section${groupIndex > 0 ? ' report-table-section--continued' : ''}">
  ...
  const tableColumns = [
    {
      ...
      minWidthMm: 32,
      align: 'left',
      render: row => escapeHtml(formatPdfMonthTitleCase(row.monthLabel)),
    },
    {
      key: 'totalHours',
      ...
      minWidthMm: 28,
      align: 'right',
      render: row => escapeHtml(formatSummaryNumber(row.totalHours)),
    },
    {
      key: 'estimatedCost',
      ...
      minWidthMm: 30,
      align: 'right',
      render: row => escapeHtml(formatSummaryCurrency(row.estimatedCost)),
    },
    {
      key: 'workCount',
      ...
      minWidthMm: 24,
      align: 'center',
      render: row => escapeHtml(String(row.workCount || '')),
    },
  ]
  const tableLayout = buildPdfReportTableLayout(tableColumns, {
    stickyFirstColumnCount: 1,
  })
  const tableSections = renderPdfReportTableSections({
    columnGroups: tableLayout.columnGroups,
    rows,
    footerRow: {
      monthLabel: 'Total anual',
      totalHours: totals.totalHours,
      estimatedCost: totals.totalEstimatedCost,
      workCount: '',
    },
  })
  ...
  return `
    ...
    <html lang="pt">
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          .report-table-section--continued {
            break-before: page;
          ...
          .report-table td {
            ...
            padding: ${tableLayout.cellPaddingY}px ${tableLayout.cellPaddingX}px;
            text-align: left;
            font-size: ${tableLayout.tableFontSizePx}px;
            line-height: 1.35;
            vertical-align: top;
            white-space: normal;
          ...
          .report-table thead th {
            ...
            font-size: ${tableLayout.headerFontSizePx}px;
          }
          ...
          @page {
            margin: ${tableLayout.topMarginMm}mm ${tableLayout.horizontalMarginMm}mm ${tableLayout.bottomMarginMm}mm;
          }
          @media print {
            body {
              padding: 0 0 28px;
      ...
      <body>
        <section class="sheet">
          <header class="header">
            <p class="report-title">RESUMO ANUAL</p>
            <h1>${escapeHtml(client?.name || 'Cliente')}</h1>
            <p class="report-year">Ano ${escapeHtml(String(year))}</p>
          </header>
...
export function WorksPageView({ forcedClientId = '', dedicatedClientView = false }) {
  ...
  const [showCompanyReportsModal, setShowCompanyReportsModal] = useState(false)
  const [summaryExportError, setSummaryExportError] = useState('')
  const [summaryExportSubmitting, setSummaryExportSubmitting] = useState(false)
  const notesTextareaRef = useRef(null)
  const annualSummarySectionRef = useRef(null)
  ...
  function closeReportsModal() {
    if (summaryExportSubmitting || exportingClientPdf) {
      return
    }
  ...
  function openCompanyReportsModal() {
    setAnnualSummaryError('')
    setShowCompanyReportsModal(true)
  }

  function closeCompanyReportsModal() {
    if (annualSummaryLoading || exportingGeneralAnnualPdf) {
      return
    }

    setShowCompanyReportsModal(false)
    setAnnualSummaryError('')
  }
 
  async function printExportedSummaryDocument(blob) {
    const url = URL.createObjectURL(blob)
    const iframe = document.createElement('iframe')
  ...
  async function handleClientSummaryExport(payload, requestedFormat = 'xlsx') {
    if (!selectedClient) {
      setSummaryExportError('Cliente não encontrado.')
      return
    }

    setSummaryExportSubmitting(true)
    setSummaryExportError('')

    try {
      const { blob, filename, contentType } = await exportClientWorkSummary(selectedClient.id, {
        ...payload,
        format: requestedFormat,
      })

      if (requestedFormat === 'pdf' || /text\/html/i.test(contentType)) {
        await printExportedSummaryDocument(blob)
      } else {
        triggerDownload(blob, filename)
      }
      setShowReportsModal(false)
    } catch (err) {
      setSummaryExportError(err.message)
    } finally {
      setSummaryExportSubmitting(false)
    }
  }

  async function openClientAnnualSummaryReport() {
    ...
    try {
      ...
      window.requestAnimationFrame(() => {
        annualSummarySectionRef.current?.scrollIntoView({ block: 'start' })
      })
    } catch (err) {
      // annualSummaryError already updated by ensureAnnualAssignmentsLoaded
    }
  }
  ...
  async function handleClientSubmit(event) {
    ...
    try {
      const payload = {
        ...
        summaryLanguage: clientForm.summaryLanguage,
      }
  ...
  return (
    <BentixPage style={pageStyle}>
      <BentixContent width="app" gap="lg" style={shellStyle}>
          ...
          >
            ...
            <div className="btx-works-hero-actions" style={{ display: 'grid', gap: '10px', justifyItems: 'end' }}>
              {dedicatedClientView ? (
                ...
                <>
                  <button type="button" onClick={openCompanyReportsModal} style={heroSecondaryButtonStyle}>
                    Relatórios
                  </button>
        ...
        {false && (
          <div ref={generalAnnualSummarySectionRef}>
          ...
          <BentixSection style={panelStyle}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <BentixButton
                type="button"
                variant="secondary"
                onClick={openCompanyReportsModal}
              >
                Relatórios
              </BentixButton>
            </div>
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
                  <div style={{ display: 'grid', gap: '8px' }}>
                    <h2 style={{ margin: 0, fontSize: '24px', lineHeight: 1.15, fontFamily: 'var(--btx-font-family)' }}>
                ...
                <div style={clientInfoGridStyle}>
                  ...
                  <div style={clientInfoCardStyle}>
                    <span style={{ fontSize: '12px', color: 'var(--vp-text-soft)', textTransform: 'uppercase' }}>Língua do resumo</span>
                    <strong>{getClientSummaryLanguageLabel(selectedClient.summaryLanguage)}</strong>
                  </div>
                </div>
        ...
        {dedicatedClientView && selectedClient && showAnnualSummary && (
          <div ref={annualSummarySectionRef}>
            <BentixSection style={panelStyle}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
              <div>
                <h2 style={{ margin: 0, fontFamily: 'var(--btx-font-family)' }}>Resumo anual</h2>
                <p style={{ margin: '8px 0 0', color: 'var(--vp-text-muted)' }}>
                  Ano de referência: <strong>{summaryYear}</strong>
                </p>
              </div>
        ...
        {dedicatedClientView && selectedClient && (
          <div className="btx-works-lists-grid" style={{ display: 'grid', gap: '24px' }}>
            <BentixSection style={panelStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap', marginBottom: '18px' }}>
                <h2 style={{ margin: 0 }}>Obras ativas</h2>
                <BentixButton
                  type="button"
                  variant="secondary"
              ...
              {!loading && !error && activeWorks.length === 0 && (
                <p>Sem obras ativas para este cliente.</p>
              )}
              {!loading && !error && activeWorks.length > 0 && (
            ...
            {!loading && archivedWorks.length > 0 && (
              <BentixSection style={panelStyle}>
                <h2 style={{ marginTop: 0 }}>Obras antigas</h2>
                <div style={{ display: 'grid', gap: '12px' }}>
      ...
      {showCompanyReportsModal && !dedicatedClientView && (
        <CompanyReportsModal
          exportingAnnualPdf={exportingGeneralAnnualPdf}
          error={annualSummaryError}
          onClose={closeCompanyReportsModal}
          onExportAnnualSummaryPdf={exportGeneralAnnualSummaryPdf}
        />
      )}

      {showReportsModal && selectedClient && (
        <ClientReportsModal
          client={selectedClient}
          works={visibleWorks}
          showAnnualSummary={showAnnualSummary}
          summaryExportSubmitting={summaryExportSubmitting}
          summaryExportError={summaryExportError}
          exportingAnnualPdf={exportingClientPdf}
          onClose={closeReportsModal}
          onOpenAnnualSummary={openClientAnnualSummaryReport}
          onExportAnnualSummaryPdf={exportClientAnnualSummaryPdf}
          onSubmitWorkedHoursSummary={handleClientSummaryExport}
        />
      )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
                ...
                <label style={labelStyle}>
                  ...
                  <select
                    ...
                    value={clientForm.summaryLanguage}
