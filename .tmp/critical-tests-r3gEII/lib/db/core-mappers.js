import { normalizeChefCategory, normalizeRole } from '../roles.js'
...
const VALID_PLANNING_PUBLICATION_STATES = new Set(['draft', 'published'])
...
export function mapClientRecord(record) {
  ...
  return {
    ...
    summaryLanguage: normalizeClientSummaryLanguage(record.summaryLanguage),
  }
}
...
export function mapPersonRecord(record) {
  ...
  return {
    ...
    chefCategory: normalizeChefCategory(record.chefCategory),
  }
}

export function normalizePlanningPublicationState(value, fallback = 'draft') {
  const normalizedValue = String(value || '').trim().toLowerCase()
  return VALID_PLANNING_PUBLICATION_STATES.has(normalizedValue) ? normalizedValue : fallback
}
...
export function mapPlanningWorkspaceRecord(record) {
  if (!record) {
    return null
  }
  ...
  return {
    ...
    companyId: Number(record.companyId),
    date: toDateOnlyString(record.date),
    state: normalizePlanningPublicationState(record.state),
    publishedWorkPlanId: toPositiveInt(record.publishedWorkPlanId, null),
    publishedAt: toDateTimeString(record.publishedAt),
