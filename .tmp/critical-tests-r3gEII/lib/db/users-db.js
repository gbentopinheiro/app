...
function mapUserRecord(record) {
  ...
  const hasExplicitAccessProfile = toPositiveInt(record.accessProfileId ?? record.accessProfile?.id, null) !== null
  const explicitPermissionKeys = Array.isArray(record.accessProfile?.permissions)
    ? record.accessProfile.permissions
  ...
  return {
    ...
    accessProfile,
    accessProfileName: toRequiredString(record.accessProfile?.name),
    accessProfileDescription: toRequiredString(record.accessProfile?.description),
    hasExplicitAccessProfile,
    permissionKeys:
      hasExplicitAccessProfile
        ? explicitPermissionKeys
        : [...getAccessProfilePermissionKeys(accessProfile)],
