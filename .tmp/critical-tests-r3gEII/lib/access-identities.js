...
import { getChefAssignmentsGrantingWorkAccess } from './chef-work-access.js'
import { isMysqlDataSourceEnabled } from './data-source.js'
import { getAllWorkAssignmentsDb } from './db/work-assignments-db.js'
import { getWorkExtraAccessWorkIdsByPerson, getWorkExtraAccessWorkIdsByPersonData } from './work-extra-access-grants.js'
import {
...
export class AccessIdentitiesService {
  ...
  getAll() {
    return this.refresh().map(identity =>
      enrichIdentity(identity, { works: resolveLegacyIdentityWorks(identity) }),
    )
  }

  getById(id) {
    const identity = this.refresh().find(item => item.id === parseInt(id))
    return identity ? enrichIdentity(identity, { works: resolveLegacyIdentityWorks(identity) }) : null
  }
...
function enrichIdentity(identity, options = {}) {
  ...
  return {
    ...identity,
    roleLabel: getEntityRoleLabel({ role: identity.role, person }),
...
function buildIdentityWorkSummaries(workIds = []) {
  return normalizeWorksIds(workIds).map(workId => {
    const work = getWorkById(workId)
    return work
      ? {
          id: work.id,
...
function resolveLegacyIdentityWorks(identity) {
  if (!identity?.personId) {
    return buildIdentityWorkSummaries(identity?.works)
  }

  return buildIdentityWorkSummaries([
    ...(Array.isArray(identity.works) ? identity.works : []),
    ...getWorkExtraAccessWorkIdsByPerson(identity.personId),
  ])
}
...
async function resolveDbIdentityWorks(user, legacyIdentity = null) {
  ...
  const extraWorkIds = await getWorkExtraAccessWorkIdsByPersonData(user.personId)
  const extraWorks = await Promise.all(extraWorkIds.map(workId => getWorkByIdData(workId)))

  const scopedAssignments = assignments.length === 0
    ? []
    : getChefAssignmentsGrantingWorkAccess(assignments).filter(
        assignment =>
  ...
  return Array.from(worksById.values()).sort((left, right) =>
