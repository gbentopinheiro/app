...
function mapDeveloperOverrideEventRecord(record) {
  if (!record) {
    return null
  }
  ...
  return {
    id: Number(record.id),
    developerUserId: record.developerUserId === null ? null : Number(record.developerUserId),
    developerUsername: String(record.developerUsername || '').trim(),
    permissionKeyUsed: String(record.permissionKeyUsed || '').trim(),
    overrideType: String(record.overrideType || '').trim(),
    entityType: String(record.entityType || '').trim(),
    entityId: record.entityId === null ? null : Number(record.entityId),
    targetDate: record.targetDate ? new Date(record.targetDate).toISOString().slice(0, 10) : null,
    ...
    errorMessage: record.errorMessage ? String(record.errorMessage).trim() : null,
    createdAt: record.createdAt ? new Date(record.createdAt).toISOString() : null,
  }
}

export async function getDeveloperOverrideEvents({ limit = 12 } = {}) {
  const normalizedLimit = Math.max(1, Math.min(Number(limit) || 12, 100))
  const records = await prisma.developerOverrideEvent.findMany({
    orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
    take: normalizedLimit,
  })

  return records.map(mapDeveloperOverrideEventRecord).filter(Boolean)
}
