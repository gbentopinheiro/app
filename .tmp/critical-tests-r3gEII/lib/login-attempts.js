...
import { clearLoginAttemptDb, getLoginAttemptByUsernameDb, upsertLoginAttemptDb } from './db/login-attempts-db.js'
...
function clearFailedLoginAttemptsJson(username) {
...
function getLoginBlockStateJson(username, now = new Date()) {
  const normalizedUsername = normalizeUsername(username)
  ...
  if (remainingMs <= 0) {
    clearFailedLoginAttemptsJson(normalizedUsername)
    return { blocked: false, retryAfterSeconds: 0 }
  }
...
function recordFailedLoginAttemptJson(username, now = new Date()) {
  const normalizedUsername = normalizeUsername(username)
...
export async function clearFailedLoginAttempts(username) {
  ...
  await clearLoginAttemptDb(normalizedUsername)
}

export async function getLoginBlockState(username, now = new Date()) {
  const normalizedUsername = normalizeUsername(username)
  ...
  if (!isMysqlDataSourceEnabled()) {
    return getLoginBlockStateJson(normalizedUsername, now)
  }

  const record = await getLoginAttemptByUsernameDb(normalizedUsername)
...
export async function recordFailedLoginAttempt(username, now = new Date()) {
  const normalizedUsername = normalizeUsername(username)
  ...
  if (!isMysqlDataSourceEnabled()) {
    return recordFailedLoginAttemptJson(normalizedUsername, now)
  }

  const currentRecord =
    (await getLoginAttemptByUsernameDb(normalizedUsername)) || {
      username: normalizedUsername,
      failedAt: [],
      blockedUntil: '',
    }
  const failedAt = [...getActiveFailures(currentRecord, now), now.toISOString()]
  const blocked = failedAt.length >= MAX_FAILED_LOGIN_ATTEMPTS

  await upsertLoginAttemptDb({
    username: normalizedUsername,
    failedAt,
    blockedUntil: blocked ? new Date(now.getTime() + LOGIN_ATTEMPT_WINDOW_MS).toISOString() : '',
  })
