...
import {
  buildResolvedChefWorkAccessMap,
  getChefAssignmentsGrantingWorkAccess,
} from './chef-work-access.js'
import {
...
export class WorkAssignment {
  constructor(data) {
    ...
    this.hoursCreatedAt = data.hoursCreatedAt || null
    this.hoursCreatedByUserId =
      data.hoursCreatedByUserId !== undefined && data.hoursCreatedByUserId !== null
        ? parseInt(data.hoursCreatedByUserId, 10)
        : null
    this.hoursCreatedByName = data.hoursCreatedByName || null
    this.hoursUpdatedAt = data.hoursUpdatedAt || null
    this.hoursUpdatedByUserId =
      data.hoursUpdatedByUserId !== undefined && data.hoursUpdatedByUserId !== null
        ? parseInt(data.hoursUpdatedByUserId, 10)
        : null
    this.hoursUpdatedByName = data.hoursUpdatedByName || null
...
export class WorkAssignmentsService {
  ...
  getAll(filters = {}) {
    ...
    const resolvedChefAccessMap = buildResolvedChefWorkAccessMap(assignments, getPersonById)
    ...
    const normalizedDateFrom = String(filters.dateFrom || '').slice(0, 10)
    const normalizedDateTo = String(filters.dateTo || '').slice(0, 10)
    ...
    return assignments
      ...
      .filter(
        assignment =>
          normalizedWorkIds.length === 0 || normalizedWorkIds.includes(Number(assignment.workId)),
      )
      ...
      .filter(assignment => {
        ...
        if (normalizedDateFrom && workPlan.date < normalizedDateFrom) {
          return false
        }

        if (normalizedDateTo && workPlan.date > normalizedDateTo) {
          return false
        }
...
function syncChefAccessForWorkPlan(assignments, workPlanId) {
  ...
  const accessGrantingAssignments = getChefAssignmentsGrantingWorkAccess(
    chefAssignments,
    getPersonById,
  )

  accessGrantingAssignments.forEach(assignment => {
    const personId = Number(assignment.personId)
    const currentWorkIds = chefWorkIdsByPerson.get(personId) || new Set()
    currentWorkIds.add(Number(assignment.workId))
...
function enrichAssignment(assignment, resolvedChefAccessMap = null) {
  ...
  return {
    ...
    assignmentPurpose: normalizeAssignmentPurpose(assignment.assignmentPurpose),
...
function validateCompanyConsistency(workPlanId, workId, personId) {
  ...
  if (Number(workPlan.companyId) !== Number(work.companyId)) {
    throw new Error('O work plan e a obra têm de pertencer à mesma empresa')
  }

  if (Number(person.companyId) !== Number(work.companyId)) {
    throw new Error('A pessoa e a obra têm de pertencer à mesma empresa')
  }
}
...
async function resolveAuditMetadata(value, actorSession = null) {
  ...
  if (!isMysqlDataSourceEnabled() || !actorSession?.username) {
    return {
      userId: null,
...
function hasHoursAuditCreator(assignment) {
  return Boolean(
    assignment?.hoursCreatedAt ||
      assignment?.hoursCreatedByUserId !== null && assignment?.hoursCreatedByUserId !== undefined ||
      assignment?.hoursCreatedByName,
  )
}

function buildHoursAuditSnapshot(assignment = null) {
  return {
    hoursCreatedAt: assignment?.hoursCreatedAt ?? null,
    hoursCreatedByUserId: assignment?.hoursCreatedByUserId ?? null,
    hoursCreatedByName: assignment?.hoursCreatedByName ?? null,
    hoursUpdatedAt: assignment?.hoursUpdatedAt ?? null,
    hoursUpdatedByUserId: assignment?.hoursUpdatedByUserId ?? null,
    hoursUpdatedByName: assignment?.hoursUpdatedByName ?? null,
  }
}

async function resolveHoursAuditFields(currentAssignment, data = {}, options = {}) {
  const currentAudit = buildHoursAuditSnapshot(currentAssignment)
  const nextAudit = {
    hoursCreatedAt:
      data.hoursCreatedAt !== undefined ? data.hoursCreatedAt : currentAudit.hoursCreatedAt,
    hoursCreatedByUserId:
      data.hoursCreatedByUserId !== undefined
        ? data.hoursCreatedByUserId
        : currentAudit.hoursCreatedByUserId,
    hoursCreatedByName:
      data.hoursCreatedByName !== undefined
        ? data.hoursCreatedByName
        : currentAudit.hoursCreatedByName,
    hoursUpdatedAt:
      data.hoursUpdatedAt !== undefined ? data.hoursUpdatedAt : currentAudit.hoursUpdatedAt,
    hoursUpdatedByUserId:
      data.hoursUpdatedByUserId !== undefined
        ? data.hoursUpdatedByUserId
        : currentAudit.hoursUpdatedByUserId,
    hoursUpdatedByName:
      data.hoursUpdatedByName !== undefined
        ? data.hoursUpdatedByName
        : currentAudit.hoursUpdatedByName,
  }
  const missingCreatorBeforeMutation = !hasHoursAuditCreator(currentAssignment)
  const shouldInitializeAuditWithActor =
    Boolean(options.actorSession) &&
    (data.hours !== undefined || (options.ensureHoursAuditWithActor === true && missingCreatorBeforeMutation))

  if (!shouldInitializeAuditWithActor) {
    return nextAudit
  }

  const auditMetadata = await resolveAuditMetadata(null, options.actorSession)
  const auditTimestamp = new Date().toISOString()

  if (!hasHoursAuditCreator(nextAudit)) {
    nextAudit.hoursCreatedAt = nextAudit.hoursCreatedAt || auditTimestamp
    nextAudit.hoursCreatedByUserId =
      nextAudit.hoursCreatedByUserId !== undefined && nextAudit.hoursCreatedByUserId !== null
        ? nextAudit.hoursCreatedByUserId
        : auditMetadata.userId
    nextAudit.hoursCreatedByName = nextAudit.hoursCreatedByName || auditMetadata.name
  }

  const shouldUpdateLastModifier =
    data.hours !== undefined || (options.ensureHoursAuditWithActor === true && missingCreatorBeforeMutation)

  if (shouldUpdateLastModifier) {
    nextAudit.hoursUpdatedAt = auditTimestamp
    nextAudit.hoursUpdatedByUserId = auditMetadata.userId
    nextAudit.hoursUpdatedByName = auditMetadata.name
  }
...
export async function createWorkAssignmentData(data, options = {}) {
  const hoursAudit = await resolveHoursAuditFields(null, data, options)
  ...
  const assignment = await createWorkAssignmentDb({
    ...
    personId: data.personId,
    hours: data.hours,
    hoursCreatedAt: hoursAudit.hoursCreatedAt,
    hoursCreatedByUserId: hoursAudit.hoursCreatedByUserId,
    hoursCreatedByName: hoursAudit.hoursCreatedByName,
    hoursUpdatedAt: hoursAudit.hoursUpdatedAt,
    hoursUpdatedByUserId: hoursAudit.hoursUpdatedByUserId,
    hoursUpdatedByName: hoursAudit.hoursUpdatedByName,
    ...
    assignmentPurpose: normalizeAssignmentPurpose(data.assignmentPurpose),
...
export async function updateWorkAssignmentData(id, data, options = {}) {
  const currentAssignment = !isMysqlDataSourceEnabled()
    ? getWorkAssignmentById(id)
    : await getWorkAssignmentByIdDb(id)

  if (!currentAssignment) {
    return null
  }

  const hoursAudit = await resolveHoursAuditFields(currentAssignment, data, options)

  if (!isMysqlDataSourceEnabled()) {
    return updateWorkAssignment(id, {
      ...data,
      ...hoursAudit,
    })
  }
  ...
  const updatedAssignment = await updateWorkAssignmentDb(id, {
    ...
    assignmentPurpose:
      data.assignmentPurpose !== undefined
        ? normalizeAssignmentPurpose(data.assignmentPurpose)
        : normalizeAssignmentPurpose(currentAssignment.assignmentPurpose),
    planningVisible:
...
export async function updateWorkAssignmentPlanningData(id, data) {
  if (!isMysqlDataSourceEnabled()) {
    ...
    return enrichAssignment(
      nextAssignment,
      buildResolvedChefWorkAccessMap(legacyService.assignments, getPersonById),
    )
  }
