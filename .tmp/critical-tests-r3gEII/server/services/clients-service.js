...
export async function createClientService(session, body) {
  ...
  const { name, vatNumber, contactName, email, phone, notes, summaryLanguage } = body || {}
  ...
  try {
    return await createClientData({ name, vatNumber, contactName, email, phone, notes, summaryLanguage })
  } catch (error) {
...
export async function updateClientService(session, id, body) {
  ...
  try {
    const client = await updateClientData(id, { name, vatNumber, contactName, email, phone, notes, summaryLanguage })
