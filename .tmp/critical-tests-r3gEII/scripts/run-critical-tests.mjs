...
const copiedEntries = ['app', 'config', 'data', 'frontend', 'infra', 'lib', 'package.json', 'public', 'scripts', 'server', 'tests']
...
const testProcess = spawnSync(
  ...
  [
    ...
    'tests/db-setup-mysql.test.mjs',
    'tests/demo-summary-cleanup.test.mjs',
    'tests/dockerfile-public-assets.test.mjs',
    'tests/pwa-update.test.mjs',
    'tests/public-app-config.test.mjs',
    'tests/session-cookie-options.test.mjs',
    'tests/work-summary-export.test.mjs',
  ],
  {
    cwd: sandboxRoot,
    stdio: 'inherit',
